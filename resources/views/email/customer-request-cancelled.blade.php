...
<tr>
    <td bgcolor="#ffffff" style="padding: 34px 30px 18px 30px; font-family: Arial, sans-serif; color: #1f2937;">
        <span style="display: inline-block; padding: 7px 12px; border-radius: 999px; background-color: #EF4444; color: #ffffff; font-size: 12px; font-weight: bold; letter-spacing: .3px; text-transform: uppercase;">
            Cancelled
        </span>
        <h1 style="margin: 14px 0 8px; font-size: 26px; line-height: 34px; color: #111827;">
            {{ $payload['headline'] ?? 'Customer request cancelled' }}
        </h1>
        <p style="margin: 0; font-size: 15px; line-height: 24px; color: #4b5563;">
            Hi {{ $data['user']['name'] ?? 'there' }}, the customer cancelled this accepted request. No further provider action is required.
        </p>
    </td>
</tr>
        ...
        <table border="0" cellpadding="0" cellspacing="0" width="100%" style="border: 1px solid #e5e7eb; border-radius: 8px;">
            <tr>
                <td style="padding: 18px 20px; font-size: 13px; line-height: 21px; color: #6b7280;">
                    ...
                    <br><br>
                    Accepted At<br>
                    <span style="color:#111827; font-size: 14px;">{{ $request['accepted_at'] ?? 'Not recorded' }}</span>
                    <br><br>
                    Cancelled At<br>
                    <span style="color:#111827; font-size: 14px;">{{ $request['cancelled_at'] ?? 'Not recorded' }}</span>
                    <br><br>
                    Description<br>
